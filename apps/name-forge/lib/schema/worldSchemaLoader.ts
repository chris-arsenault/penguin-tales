...
export function loadWorldSchema(schemaPath?: string): WorldSchema {
  ...
  const pathsToTry = schemaPath ? [schemaPath] : [
    // Relative to working directory (apps/name-forge/)
    join(process.cwd(), 'worldSchema.json'),
    // Sibling world-gen project (from apps/name-forge/ to world-gen/)
    join(process.cwd(), '../../world-gen/worldSchema.json'),
    // From dist/lib/schema to project root
    join(__dirname, '../../../worldSchema.json'),
    // From dist/lib/schema up to penguin-tales/world-gen
    join(__dirname, '../../../../../world-gen/worldSchema.json'),
  ];
  ...
  throw new Error(`World schema not found at any of: ${pathsToTry.join(', ')}`);
}
