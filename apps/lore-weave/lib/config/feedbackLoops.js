...
export function getNegativeFeedbackLoops() {
    return feedbackLoops.filter(loop => loop.type === 'negative');
}
export function getPositiveFeedbackLoops() {
    return feedbackLoops.filter(loop => loop.type === 'positive');
}
/**
 * Get loops involving a specific entity or pressure
 */
export function getLoopsForTarget(target) {
    return feedbackLoops.filter(loop => loop.source.includes(target) || loop.target.includes(target));
}
//# sourceMappingURL=feedbackLoops.js.map