...
export class EnrichmentService {
    ...
    config;
    loreLog = [];
    constructor(llmConfig, loreProvider, config) {
     ...
     */
    async batchGenerateNames(entities, context) {
    ...
    async enrichEntities(entities, context, options) {
        if (!this.isEnabled() || entities.length === 0) {
            return [];
        }
        const records = [];
        ...
        for (const batch of batches) {
                ...
                if (snapshotEntity) {
                    ...
                    snapshotEntity.links.forEach(link => {
                        if (link.status === 'historical')
                            return; // Skip historical here
                        const target = context.graphSnapshot.entities.get(link.dst);
                        if (target) {
                            if (!relationships[link.kind])
                                relationships[link.kind] = [];
                    ...
                    if (context.graphSnapshot.historicalRelationships) {
                        context.graphSnapshot.historicalRelationships
                            ...
                            if (target) {
                                if (!historicalRelationships[link.kind])
                                    historicalRelationships[link.kind] = [];
            ...
            let parsed = [];
            const parsedResult = parseJsonSafe(result.text);
            if (parsedResult) {
                parsed = parsedResult;
            }
            else if (result.text) {
    ...
    async generateEraNarrative(params) {
        if (!this.isEnabled())
            return null;
        const prompt = [
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse era narrative response');
            return null;
        }
        const record = {
    ...
    async enrichRelationships(relationships, actors, context) {
        if (!this.isEnabled() || relationships.length === 0)
            return [];
        const records = [];
    ...
    async enrichAbility(ability, context) {
        if (!this.isEnabled())
            return null;
    ...
    async enrichDiscoveryEvent(params) {
        if (!this.isEnabled())
            return null;
        let contextClue = '';
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse discovery event response');
            return null;
        }
        const record = {
    ...
    async generateChainLink(params) {
        if (!this.isEnabled())
            return null;
        const geoConstraints = this.loreProvider.getGeographyConstraints();
        const prompt = [
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse chain link response');
            return null;
        }
        const record = {
    ...
    async enrichOccurrence(occurrence, context) {
        if (!this.isEnabled())
            return null;
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse occurrence enrichment response');
            return null;
        }
    ...
    async enrichEra(era, context) {
        ...
        const eraOccurrences = Array.from(context.graphSnapshot.entities.values())
            ...
            context.graphSnapshot.relationships.some(r => r.kind === 'active_during' && r.src === e.id && r.dst === era.id))
            .map(e => e.name);
        // Get domain-specific prompt template
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse era enrichment response');
            return null;
        }
    ...
    async enrichEntityChanges(entity, changes, context) {
        if (!this.isEnabled())
            return null;
        const prompt = [
        ...
        if (!parsed) {
            if (result.text)
                console.warn('Failed to parse change enrichment response');
            return null;
        }
        const record = {
