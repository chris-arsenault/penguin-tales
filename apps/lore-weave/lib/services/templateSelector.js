...
export class TemplateSelector {
     ...
     */
    selectTemplates(graph, availableTemplates, eraWeights, count) {
        ...
        const guidedWeights = this.calculateGuidedWeights(availableTemplates, eraWeights, state, deviation);
     ...
     */
    calculateGuidedWeights(templates, eraWeights, state, deviation) {
        ...
        templates.forEach((template) => {
            ...
            if (deviation.entityKind.score > tuning.convergenceThreshold) {
                metadata.produces.entityKinds.forEach((kindInfo) => {
                    ...
                    const targetRatio = this.targets.global.entityKindDistribution.targets[kindInfo.kind] || 0;
            ...
            if (deviation.prominence.score > tuning.convergenceThreshold) {
                metadata.produces.entityKinds.forEach((kindInfo) => {
                    kindInfo.prominence.forEach((promInfo) => {
                        ...
                        const targetRatio = this.targets.global.prominenceDistribution.targets[promInfo.level] || 0;
                        if (promDeviation !== undefined && promInfo.probability > 0.3) {
                            if (currentRatio < targetRatio) {
                                const boost = 1 + promDeviation * tuning.correctionStrength.prominence;
                                adjustedWeight *= boost;
                                adjustmentReasons.push(`+${((boost - 1) * 100).toFixed(0)}% (need more ${promInfo.level})`);
                            }
                            else if (currentRatio > targetRatio) {
                                const penalty = 1 - promDeviation * tuning.correctionStrength.prominence * 0.5;
                                adjustedWeight *= Math.max(0.2, penalty);
                                adjustmentReasons.push(`${((penalty - 1) * 100).toFixed(0)}% (too many ${promInfo.level})`);
                            }
                        }
                    });
                });
                ...
                if (maxRatio > targetMaxRatio) {
                    ...
                    metadata.produces.relationships.forEach((relInfo) => {
                        ...
                        if (currentRatio > targetMaxRatio * 0.8) {
                            ...
                            adjustmentReasons.push(`${((penalty - 1) * 100).toFixed(0)}% (${relInfo.kind} over-represented)`);
                        }
                    });
                }
            ...
            // Clamp to min/max
            adjustedWeight = Math.max(tuning.minTemplateWeight, Math.min(tuning.maxTemplateWeight, adjustedWeight));
