...
export class FeedbackAnalyzer {
    ...
    validateLoop(loop, metrics) {
        ...
        const correlation = this.calculateCorrelation(source.history, target.history, loop.delay);
    ...
    parseMetricPath(path, metrics) {
        ...
        if (identifier.includes(':')) {
            ...
            return metric ? { history: metric.history, value: metric[field] } : null;
        }
        // Check if it's a relationship metric
        const relMetric = metrics.relationships.get(identifier);
        if (relMetric) {
            return { history: relMetric.history, value: relMetric[field] };
        }
        // Check if it's a pressure metric
        const pressureMetric = metrics.pressures.get(identifier);
        if (pressureMetric) {
            return { history: pressureMetric.history, value: pressureMetric[field] };
        }
        return null;
    }
    /**
     ...
     */
    calculateCorrelation(x, y, delay = 0) {
     ...
     */
    updateLoopStrength(id, newStrength) {
     ...
     */
    generateDetailedDiagnostics(brokenLoop) {
        const diagnostics = [];
        if (!this.config) {
     ...
     */
    printDetailedDiagnostics(results) {
...
//# sourceMappingURL=feedbackAnalyzer.js.map