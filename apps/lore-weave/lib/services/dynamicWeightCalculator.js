...
export class DynamicWeightCalculator {
    deviationThreshold = 0.2; // 20% deviation triggers adjustment
    maxSuppressionFactor = 0.8; // Can suppress down to 20% of base
    maxBoostFactor = 2.0; // Can boost up to 200% of base
    /**
     * Calculate adjusted weight for a template based on population metrics
     */
    calculateWeight(template, baseWeight, metrics) {
        if (baseWeight === 0) {
            return {
        ...
        produces.entityKinds.forEach(entityKind => {
            ...
            if (deviation > this.deviationThreshold) {
                ...
                reasons.push(`${key} over target by ${(deviation * 100).toFixed(0)}% (${metric.count}/${metric.target}), suppressing by ${((1 - suppressionFactor) * 100).toFixed(0)}%`);
            }
                ...
                reasons.push(`${key} under target by ${(Math.abs(deviation) * 100).toFixed(0)}% (${metric.count}/${metric.target}), boosting by ${((boostFactor - 1) * 100).toFixed(0)}%`);
            }
        });
     ...
     */
    calculateAllWeights(templates, baseWeights, metrics) {
     ...
     */
    getSuppressedTemplates(adjustments) {
     ...
     */
    getBoostedTemplates(adjustments) {
        return Array.from(adjustments.values())
...
//# sourceMappingURL=dynamicWeightCalculator.js.map