 ...
 * // { npc: 2, faction: 1 }
 */
export function calculateEntityKindCounts(entities) {
    const counts = {};
    entities.forEach(e => {
 ...
 */
export function calculateRatios(counts, total) {
    const ratios = {};
 ...
 */
export function calculateProminenceDistribution(entities) {
    const counts = {
        forgotten: 0,
        marginal: 0,
        recognized: 0,
 ...
 */
export function calculateRelationshipDistribution(graph) {
    const counts = {};
 ...
 */
export function calculateConnectivityMetrics(graph) {
    const connectionCounts = new Map();
    graph.relationships.forEach(r => {
        connectionCounts.set(r.src, (connectionCounts.get(r.src) || 0) + 1);
        connectionCounts.set(r.dst, (connectionCounts.get(r.dst) || 0) + 1);
    });
    const isolatedNodes = Array.from(graph.entities.keys()).filter(id => !connectionCounts.has(id)).length;
 ...
 */
export function calculateSubtypeDistribution(entities) {
    const counts = {};
    entities.forEach(e => {
...
//# sourceMappingURL=distributionCalculations.js.map