...
import { getAgentsByCategory, calculateAttemptChance, addCatalyzedEvent, updateInfluence, getInfluence } from '../utils/catalystHelpers';
/**
 ...
 */
export const universalCatalyst = {
    ...
    apply: (graph, modifier = 1.0) => {
        ...
        allAgents.forEach(agent => {
            ...
            if (Math.random() > finalAttemptChance)
                return;
            actionsAttempted++;
 ...
 */
function getRelevantPressures(graph, actionDomains) {
 ...
 */
function selectAction(agent, graph, actionDomains) {
    if (!agent.catalyst)
        return null;
    // Find actions available to this agent
    const availableActions = [];
    actionDomains.forEach(domain => {
        if (!agent.catalyst?.actionDomains.includes(domain.id))
            return;
 ...
 */
function meetsRequirements(agent, action, graph) {
    ...
    if (reqs.requiredRelationships) {
        const hasAll = reqs.requiredRelationships.every((relKind) => agent.links.some(link => link.kind === relKind));
        if (!hasAll)
            return false;
    }
 ...
 */
function calculateActionWeight(action, domain, graph) {
    ...
    relevantPressures.forEach((pressureId) => {
        const pressure = graph.pressures.get(pressureId) || 0;
        if (pressure > 50) {
 ...
 */
function executeAction(agent, action, graph) {
...
//# sourceMappingURL=universalCatalyst.js.map