 ...
 */
export const relationshipCulling = {
    ...
    apply: (graph, modifier = 1.0) => {
        ...
        graph.relationships = graph.relationships.filter(rel => {
            ...
            const age = Math.min(graph.tick - srcEntity.createdAt, graph.tick - dstEntity.createdAt);
            ...
            if (strength < cullThreshold) {
                ...
                if (srcEntity) {
                    srcEntity.links = srcEntity.links.filter(l => !(l.kind === rel.kind && l.src === rel.src && l.dst === rel.dst));
...
//# sourceMappingURL=relationshipCulling.js.map