...
export const eraTransition = {
    ...
    apply: (graph, modifier = 1.0) => {
        ...
        const currentEra = Array.from(graph.entities.values()).find(e => e.kind === 'era' && e.status === 'current');
        if (!currentEra) {
        ...
        const nextEra = Array.from(graph.entities.values()).find(e => e.kind === 'era' && e.status === 'future');
        if (!nextEra) {
        ...
        const prominentEntities = Array.from(graph.entities.values()).filter(e => (e.prominence === 'recognized' || e.prominence === 'renowned' || e.prominence === 'mythic') &&
            e.kind !== 'era' &&
 ...
 */
function activateFirstEra(graph) {
    const firstEra = Array.from(graph.entities.values()).find(e => e.kind === 'era');
    if (!firstEra) {
        return {
            relationshipsAdded: [],
            entitiesModified: [],
 ...
 */
function checkTransitionConditions(currentEra, graph) {
    ...
    return transitionConditions.every((condition) => {
 ...
 */
function checkDefaultTransitionConditions(currentEra, graph) {
 ...
 */
function checkPressureCondition(condition, graph) {
 ...
 */
function checkEntityCountCondition(condition, graph) {
    const entities = Array.from(graph.entities.values()).filter(e => e.kind === condition.entityKind &&
 ...
 */
function checkOccurrenceCondition(condition, graph) {
    const occurrences = Array.from(graph.entities.values()).filter(e => e.kind === 'occurrence' &&
        (!condition.subtype || e.subtype === condition.subtype) &&
        e.status === 'active');
    switch (condition.operator) {
        case 'exists':
        ...
        case 'ended':
            const endedOccurrences = Array.from(graph.entities.values()).filter(e => e.kind === 'occurrence' &&
 ...
 */
function checkTimeCondition(condition, currentEra, graph) {
    const eraAge = currentEra.temporal
        ? graph.tick - currentEra.temporal.startTick
        : graph.tick - currentEra.createdAt;
    return eraAge > condition.minTicks;
}
//# sourceMappingURL=eraTransition.js.map