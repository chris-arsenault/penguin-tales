import { generateId, addEntity, addRelationship, updateEntity, pickRandom, weightedRandom, findEntities, getProminenceValue, upsertNameTag } from '../utils/helpers';
import { initializeCatalystSmart } from '../utils/catalystHelpers';
import { getTemplateWeight, getSystemModifier } from '../utils/eraUtils';
 ...
 */
function detectLocationChanges(location, snapshot, graph) {
    const changes = [];
    // Population changes (track as dst of resident_of)
    const currentResidents = graph.relationships.filter(r => r.kind === 'resident_of' && r.dst === location.id);
    ...
    const currentController = graph.relationships.find(r => (r.kind === 'stronghold_of' || r.kind === 'controls') && r.dst === location.id);
 ...
 */
function detectFactionChanges(faction, snapshot, graph) {
    const changes = [];
    // Leadership changes (track as dst of leader_of)
    const currentLeader = graph.relationships.find(r => r.kind === 'leader_of' && r.dst === faction.id);
    ...
    const currentTerritories = graph.relationships.filter(r => (r.kind === 'stronghold_of' || r.kind === 'controls') && r.src === faction.id);
    ...
    const currentAllies = new Set(graph.relationships
    ...
    newAllies.forEach(allyId => {
        ...
        if (ally)
            changes.push(`alliance: allied with ${ally.name}`);
    });
    // War changes (track as src of at_war_with)
    const currentEnemies = new Set(graph.relationships
    ...
    newWars.forEach(enemyId => {
        ...
        if (enemy)
            changes.push(`war: declared war on ${enemy.name}`);
    });
 ...
 */
function detectRuleChanges(rule, snapshot, graph) {
    const changes = [];
    // Only enrich if rule is prominent
    const prominenceValue = getProminenceValue(rule.prominence);
    if (prominenceValue < 2)
        return changes; // Skip if not 'recognized' or higher
    ...
    const currentEnforcers = new Set(graph.relationships
 ...
 */
function detectAbilityChanges(ability, snapshot, graph) {
    const changes = [];
    // Practitioner count changes (track as dst of practitioner_of)
    const currentPractitioners = graph.relationships.filter(r => r.kind === 'practitioner_of' && r.dst === ability.id);
    ...
    const currentLocations = new Set(graph.relationships
 ...
 */
function detectNPCChanges(npc, snapshot, graph) {
    const changes = [];
    // Only track renowned/mythic NPCs
    const prominenceValue = getProminenceValue(npc.prominence);
    if (prominenceValue < 3)
        return changes; // Skip if not 'renowned' or 'mythic'
    // Leadership changes (track as src of leader_of)
    const currentLeaderships = new Set(npc.links
...
export class WorldEngine {
    ...
    templateSelector; // Optional statistical template selector
    systemSelector; // Optional statistical system weighting
    distributionTracker; // Distribution measurement
    statisticsCollector; // Statistics tracking for fitness evaluation
    populationTracker; // Population metrics for homeostatic control
    dynamicWeightCalculator; // Dynamic template weight adjustment
    feedbackAnalyzer; // Feedback loop validation
    contractEnforcer; // Active contract enforcement
    pendingEnrichments = [];
    pendingNameEnrichments = [];
    entityEnrichmentsUsed = 0;
    relationshipEnrichmentsUsed = 0;
    eraNarrativesUsed = 0;
    // Entity enrichment batching queue
    entityEnrichmentQueue = [];
    ENRICHMENT_BATCH_SIZE = 15; // Batch size for accumulating entities
    // Engine-level safeguards
    systemMetrics = new Map();
    lastRelationshipCount = 0;
    warningLogPath;
    // Template diversity tracking
    templateRunCounts = new Map();
    ...
    metaEntitiesFormed = [];
    constructor(config, initialState, enrichmentService, imageGenerationService) {
        this.config = config;
        ...
        this.populationTracker = new PopulationTracker(config.distributionTargets || {}, config.domain);
    ...
    linkFinalEra() {
        ...
        const currentEra = Array.from(this.graph.entities.values()).find(e => e.kind === 'era' && e.status === 'current');
        if (!currentEra || !currentEra.temporal)
            return;
        ...
        const prominentEntities = Array.from(this.graph.entities.values()).filter(e => (e.prominence === 'recognized' || e.prominence === 'renowned' || e.prominence === 'mythic') &&
            e.kind !== 'era' &&
    ...
    runEpoch() {
        ...
        this.statisticsCollector.recordEpoch(this.graph, this.currentEpoch, entitiesCreated, relationshipsCreated, growthTargets);
    ...
    runGrowthPhase(era, growthTargets) {
        ...
        while (entitiesCreated < targets && attempts < maxAttempts) {
            ...
            try {
                ...
                const lineageRelationships = this.contractEnforcer.enforceLineage(this.graph, graphView, clusterEntities);
                ...
                const warnings = this.contractEnforcer.validateAffects(template, result.entities.length, result.relationships.length + lineageRelationships.length, new Map() // TODO: Track pressure changes
                );
                ...
                const allRelationships = [...result.relationships, ...lineageRelationships];
                // Record history
                this.graph.history.push({
     ...
     */
    sampleSingleTemplate(era, applicableTemplates, metrics) {
        if (applicableTemplates.length === 0)
            return undefined;
        // Build era weights
        const baseEraWeights = {};
        ...
        const adjustments = this.dynamicWeightCalculator.calculateAllWeights(applicableTemplates, new Map(Object.entries(baseEraWeights)), metrics);
    ...
    calculateGrowthTarget() {
        ...
        if (totalRemaining === 0)
            return 3; // Minimal growth when minimum target met
     ...
     */
    selectWeightedTemplates(era, deficits, growthTarget) {
        ...
        for (const template of this.config.templates) {
            ...
            if (eraWeight === 0)
                continue; // Template disabled in this era
    ...
    runSimulationTick(era) {
        let totalRelationships = 0;
        let totalModifications = 0;
        const relationshipsThisTick = [];
        ...
        for (const system of this.config.systems) {
            ...
            if (baseModifier === 0)
                continue; // System disabled by era
     ...
     */
    calculateDistributionSystemModifiers(era) {
        ...
        return this.systemSelector.calculateSystemModifiers(this.graph, this.config.systems, eraModifiers);
    }
    /**
     ...
     */
    calculateDistributionPressureAdjustments() {
    ...
    pruneAndConsolidate() {
        ...
        for (const entity of this.graph.entities.values()) {
            if (entity.prominence === 'forgotten')
                continue;
            const age = this.graph.tick - entity.createdAt;
            const connections = this.graph.relationships.filter(r => r.src === entity.id || r.dst === entity.id).length;
    ...
    reportDistributionStats() {
        ...
        Object.entries(state.prominenceRatios).forEach(([level, ratio]) => {
            const target = this.config.distributionTargets?.global.prominenceDistribution.targets[level] || 0;
            const delta = ratio - target;
            const sign = delta > 0 ? '+' : '';
            const status = Math.abs(delta) > 0.05 ? '⚠️' : '✓';
    ...
    async waitForNameEnrichmentsSnapshot() {
        if (this.pendingNameEnrichments.length === 0)
            return;
    ...
    enrichInitialEntities() {
        ...
        const enrichmentPromise = (async () => {
            const records = await this.enrichmentService.enrichEntities(initialEntities, context, { preserveNames: true } // Keep canonical names
            );
    ...
    queueEntityEnrichment(entities) {
        if (!this.enrichmentService?.isEnabled() || entities.length === 0)
            return;
    ...
    flushEntityEnrichmentQueue(force = false) {
        if (!this.enrichmentService?.isEnabled())
            return;
        if (this.entityEnrichmentQueue.length === 0)
            return;
        // Only flush if we have enough entities, unless forced
        if (!force && this.entityEnrichmentQueue.length < this.ENRICHMENT_BATCH_SIZE)
            return;
        ...
        const enrichmentPromise = this.waitForNameEnrichmentsSnapshot().then(async () => {
            ...
            const abilityRecords = await Promise.all(entitiesToEnrich
    ...
    queueRelationshipEnrichment(newRelationships) {
        if (!this.enrichmentService?.isEnabled())
            return;
     ...
     */
    queueOccurrenceEnrichment(occurrence, catalystId) {
        if (!this.enrichmentService?.isEnabled())
            return;
        const context = this.buildEnrichmentContext();
        ...
        const enrichmentPromise = (async () => {
            ...
            if (record) {
                ...
                occurrence.name = record.metadata?.name || occurrence.name;
    ...
    snapshotEntity(entity) {
        const keyRelationshipIds = new Set(entity.links
        ...
        switch (entity.kind) {
            case 'location':
                ...
                snapshot.residentCount = this.graph.relationships.filter(r => r.kind === 'resident_of' && r.dst === entity.id).length;
                // Controller (faction with stronghold_of or controls pointing to this location)
                const controller = this.graph.relationships.find(r => (r.kind === 'stronghold_of' || r.kind === 'controls') && r.dst === entity.id);
                snapshot.controllerId = controller?.src;
                break;
            case 'faction':
                // Leader (who has leader_of pointing to this faction)
                const leader = this.graph.relationships.find(r => r.kind === 'leader_of' && r.dst === entity.id);
                snapshot.leaderId = leader?.src;
                // Territory count (stronghold_of or controls from this faction)
                snapshot.territoryCount = this.graph.relationships.filter(r => (r.kind === 'stronghold_of' || r.kind === 'controls') && r.src === entity.id).length;
                // Allies (allied_with from this faction)
                snapshot.allyIds = new Set(this.graph.relationships
                    .filter(r => r.kind === 'allied_with' && r.src === entity.id)
                    .map(r => r.dst));
                // Enemies (at_war_with from this faction)
                snapshot.enemyIds = new Set(this.graph.relationships
                    .filter(r => r.kind === 'at_war_with' && r.src === entity.id)
                    .map(r => r.dst));
                break;
            case 'rules':
                // Enforcers (factions with weaponized_by or kept_secret_by pointing to this rule)
                snapshot.enforcerIds = new Set(this.graph.relationships
            ...
            case 'abilities':
                ...
                snapshot.practitionerCount = this.graph.relationships.filter(r => r.kind === 'practitioner_of' && r.dst === entity.id).length;
                // Locations where it manifests (manifests_at from this ability)
                snapshot.locationIds = new Set(this.graph.relationships
                    .filter(r => r.kind === 'manifests_at' && r.src === entity.id)
                    .map(r => r.dst));
                break;
            case 'npc':
                // Leadership positions (leader_of from this NPC)
                snapshot.leadershipIds = new Set(entity.links
                    .filter(l => l.kind === 'leader_of')
                    .map(l => l.dst));
                break;
        }
    ...
    queueEraNarrative(fromEra, toEra) {
        if (!this.enrichmentService?.isEnabled())
            return;
        if (fromEra.id === toEra.id)
            return;
        if (this.config.enrichmentConfig?.mode === 'partial') {
            const limit = this.config.enrichmentConfig?.maxEraNarratives ?? 0;
            if (this.eraNarrativesUsed >= limit)
                return;
        }
        if (this.config.enrichmentConfig?.mode === 'partial') {
    ...
    queueDiscoveryEnrichment(entities) {
        if (!this.enrichmentService?.isEnabled())
            return;
        ...
        discoveries.forEach(location => {
            ...
            const discoveryRel = this.graph.relationships.find(r => (r.kind === 'discovered_by' && r.src === location.id) ||
            ...
            if (adjacentLocations.length > 0) {
                const recentlyDiscovered = adjacentLocations.find(loc => (this.graph.tick - loc.createdAt) <= 20);
    ...
    async generateMythicImages() {
        ...
        const results = await this.imageGenerationService.generateImagesForMythicEntities(entities, context);
     ...
     */
    exportStatistics(validationResults) {
        return this.statisticsCollector.generateStatistics(this.graph, this.config, {
