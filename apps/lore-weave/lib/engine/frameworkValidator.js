...
export class FrameworkValidator {
     ...
     */
    validateCoverage(errors, warnings) {
     ...
     */
    validateEquilibrium(errors, warnings) {
        for (const pressure of this.config.pressures) {
            if (!pressure.contract) {
            ...
            if (pressure.decay > 0) {
                ...
                if (predictedEquilibrium < expectedMin * 0.8 || predictedEquilibrium > expectedMax * 1.2) {
                    warnings.push(`Pressure '${pressure.name}' equilibrium mismatch: ` +
     ...
     */
    validateAchievability(errors, warnings) {
        ...
        for (const registry of this.config.entityRegistries) {
            ...
            if (estimatedCapacity < registry.expectedDistribution.targetCount * 0.5) {
                warnings.push(`Entity kind '${registry.kind}' may not reach target count ${registry.expectedDistribution.targetCount} ` +
            ...
            if (Math.abs(prominenceSum - 1.0) > 0.01) {
                errors.push(`Entity kind '${registry.kind}' prominence distribution sums to ${prominenceSum.toFixed(2)}, ` +
                    `expected 1.0`);
            }
        }
    }
     ...
     */
    validateContracts(errors, warnings) {
        ...
        for (const template of this.config.templates) {
            ...
            if (template.contract.purpose !== ComponentPurpose.ENTITY_CREATION &&
                ...
                warnings.push(`Template '${template.id}' has purpose ${template.contract.purpose}, ` +
        ...
        for (const system of this.config.systems) {
            ...
            if (system.contract.purpose === ComponentPurpose.ENTITY_CREATION) {
                errors.push(`System '${system.id}' has purpose ENTITY_CREATION - ` +
