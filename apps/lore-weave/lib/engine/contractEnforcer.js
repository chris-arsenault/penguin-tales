...
export class ContractEnforcer {
     ...
     */
    checkContractEnabledBy(template, graph, graphView) {
        ...
        if (enabledBy.entityCounts) {
            for (const ec of enabledBy.entityCounts) {
                const criteria = { kind: ec.kind };
                if (ec.subtype)
                    criteria.subtype = ec.subtype;
     ...
     */
    enforceLineage(graph, graphView, newEntities) {
        ...
        for (const entity of newEntities) {
            ...
            if (ancestor) {
                ...
                // Add to graph
                addRelationship(graph, registry.lineage.relationshipKind, entity.id, ancestor.id, undefined, distance);
                // Track for history
     ...
     */
    checkSaturation(template, graph) {
        ...
        const saturatedKinds = [];
        const unsaturatedKinds = [];
        for (const kindInfo of entityKinds) {
            // Try to find subtype-specific registry first, fall back to kind registry
            let registry = this.config.entityRegistries?.find(r => r.kind === kindInfo.kind && r.subtype === kindInfo.subtype);
            if (!registry) {
                registry = this.config.entityRegistries?.find(r => r.kind === kindInfo.kind && !r.subtype);
            }
            if (!registry) {
            ...
            const criteria = { kind: kindInfo.kind };
     ...
     */
    validateAffects(template, entitiesCreated, relationshipsCreated, pressureChanges) {
        const warnings = [];
            ...
            for (const entityAffect of affects.entities) {
                if (entityAffect.operation === 'create' && entityAffect.count) {
                    ...
                    if (entitiesCreated < min) {
                        warnings.push(`Created ${entitiesCreated} entities, but contract requires min ${min}`);
                    }
                    if (entitiesCreated > max) {
                        warnings.push(`Created ${entitiesCreated} entities, but contract allows max ${max}`);
                    }
                }
            }
        }
        ...
        if (affects.relationships) {
            ...
            if (relationshipsCreated > totalExpectedMax * 1.2) { // Allow 20% overshoot
                warnings.push(`Created ${relationshipsCreated} relationships, but contract suggests max ~${totalExpectedMax}`);
            }
        }
        ...
        if (affects.pressures) {
            for (const pressureAffect of affects.pressures) {
                if (pressureAffect.delta !== undefined) {
                    ...
                    if ((pressureAffect.delta > 0 && actualChange < 0) ||
                        (pressureAffect.delta < 0 && actualChange > 0)) {
                        warnings.push(`Pressure '${pressureAffect.name}' changed ${actualChange.toFixed(1)}, but contract declares ${pressureAffect.delta}`);
                    }
                }
            }
        }
    ...
    checkTagSaturation(graph, tagsToAdd) {
        ...
        for (const tag of tagsToAdd) {
            ...
            if (!def || !def.maxUsage)
                continue; // Unregistered tags can't be saturated
     ...
     */
    enforceTagCoverage(entity, graph) {
     ...
     */
    getDiagnostic(template, graph, graphView) {
        const parts = [];
        // Check contract
