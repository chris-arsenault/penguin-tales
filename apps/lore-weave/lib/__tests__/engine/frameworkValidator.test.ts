...
describe('FrameworkValidator', () => {
  ...
  describe('validateContracts', () => {
    ...
    // Lineage is handled via inline lineage specs on CreationRule in declarative templates

    it('should not warn for templates without contracts', () => {
      mockConfig.templates = [
        {
      ...
      // Contracts are now optional - no warning expected
      expect(result.warnings).not.toContainEqual(
        expect.stringContaining('has no contract')
      );
    });
