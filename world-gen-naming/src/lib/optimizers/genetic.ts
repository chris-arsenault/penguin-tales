...
export async function geneticAlgorithm(
  ...
  for (let i = 0; i < gaSettings.populationSize; i++) {
    // Create varied individuals through mutation
    const mutationCount = i === 0 ? 0 : Math.floor(rng() * 5) + 1;
    const genome = applyMultipleMutations(initialDomain, mutationCount, rng);
    ...
    process.stdout.write(`\r  Initialized ${i + 1}/${gaSettings.populationSize}`);
  }
  console.log();
  ...
  for (let gen = 0; gen < generations; gen++) {
    ...
    while (newPopulation.length < gaSettings.populationSize) {
      // Selection
      ...
      // Evaluate children
      const eval1 = await computeFitness(
        child1,
        { consonantWeights: [], vowelWeights: [], templateWeights: [], structureWeights: [], apostropheRate: 0, hyphenRate: 0, lengthMin: 0, lengthMax: 0 },
        validationSettings,
        fitnessWeights,
      ...
      newPopulation.push({
        genome: child1,
        fitness: eval1.fitness,
        scores: eval1.scores,
      });

      evaluations.push(eval1);

      if (newPopulation.length < gaSettings.populationSize) {
        const eval2 = await computeFitness(
          child2,
          { consonantWeights: [], vowelWeights: [], templateWeights: [], structureWeights: [], apostropheRate: 0, hyphenRate: 0, lengthMin: 0, lengthMax: 0 },
          validationSettings,
          fitnessWeights,
